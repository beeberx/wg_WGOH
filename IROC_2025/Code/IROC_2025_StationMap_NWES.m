...
%%
for ii=1:16
    if ismember(IROC_metaData.Index{ii},{'NWES_001','NWES_005','NWES_016'})
        continue
    elseif ismember(IROC_metaData.Index{ii},{'NWES_004'})
        txtT = {'NWES_004';'NWES_005'};
        posT = [LONT(ii)-1,LATT(ii)];
        tHal = 'right';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_014'})
        txtT = {'NWES_014';'NWES_016'};
        posT = [LONT(ii)+1.5,LATT(ii)+0.75];
        tHal = 'left';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_009'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii)-1,LATT(ii)];
        tHal = 'right';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_015'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii)+1.5,LATT(ii)];
        tHal = 'left';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_002'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii),LATT(ii)-1];
        tHal = 'left';
        tVal = 'top';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_006'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii)+2,LATT(ii)-1];
        tHal = 'left';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_011'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii),LATT(ii)-0.5];
        tHal = 'center';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_012'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii),LATT(ii)-1.3];
        tHal = 'center';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_010'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii)+.2,LATT(ii)+0.5];
        tHal = 'left';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_008'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii),LATT(ii)+0.5];
        tHal = 'right';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_017'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii),LATT(ii)+1];
        tHal = 'left';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_003'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii)-3,LATT(ii)];
        tHal = 'right';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_013'})
        txtT = IROC_metaData.Index{ii};
        posT = [LONT(ii)-1,LATT(ii)-1];
        tHal = 'right';
        tVal = 'middle';
    elseif ismember(IROC_metaData.Index{ii},{'NWES_007'})
        txtT = IROC_metaData.Index{ii};
