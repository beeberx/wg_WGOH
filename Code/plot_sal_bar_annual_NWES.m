...
% define data paths
IROC_data_path= ['../IROC_Timeseries/'];
figpath=[pwd,filesep];
...
timeseries_files = {'Plymouth_WCO_E1_Annual.csv';
...
ph = floor((0.93-(pnum*deltaplot))/pnum.*100)./100;
pos_mat(:,1)=0.05;
...
for ss=1:size(timeseries_files,1);
    ...
    sdata = tmp(:,7);% standardised anomalies
    clear tmp
...
for pp=1:size(pgc,1)
    if pp==1
        patch(tx,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 1 1 0],pgc(pp,:),'edgecolor','none')
        text(pp,yytxt,{'';'y';['< ' sprintf('% -3.1f',cbounds(pp))]}, ...
            'horizontalalignment','center','VerticalAlignment','bottom','interpreter','tex','FontSize',10);
        patch(tx,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 6 6 0],pgc(pp,:),'facecolor','none','edgecolor','k')
    elseif pp==size(pgc,1)
        patch(tx,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 1 1 0],pgc(pp,:),'edgecolor','none')
